...
class Meteorite(pygame.sprite.Sprite):
    ...
    def turn(self, meteorite, index):
        ...
        if(pygame.Rect.colliderect(row1, meteorite.rect) or pygame.Rect.colliderect(row2, meteorite.rect) 
        or pygame.Rect.colliderect(col1, meteorite.rect) or (pygame.Rect.colliderect(col2, meteorite.rect) 
        or pygame.Rect.colliderect(col3, meteorite.rect))):
...
def createMeteorite():
    ...
    index = 0
    meteorite = Meteorite(IMAGEWIDTH, IMAGEHEIGHT, x, y, index) #1st meteorite initialize
    x2, y2 = 215, 215  #2nd隕石的起始位置
    index2 = 3
    meteorite2 = Meteorite(IMAGEWIDTH, IMAGEHEIGHT, x2, y2, index2)
...
def reloadMeteorite():
    ...
    for meteorite in Meteorite_list:
        ...
        new_meteorite = Meteorite(IMAGEWIDTH, IMAGEHEIGHT, x, y, index)
...
def playBGM():
    ...
    pygame.mixer.music.load("through-the-space.mp3")
...
reload_meteorite_event = USEREVENT + 1
pygame.time.set_timer(reload_meteorite_event, 300)
...
while run:
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            run = False
        elif event.type == reload_meteorite_event:
            reloadMeteorite()
    #背景黑色
